...
class RNNStateEncoder(nn.Module):
    def __init__(
        ...
        self.encoder = resnet18(pretrained=True, num_classes=hidden_size)
    ...
    def seq_forward(self, x, hidden_states, masks):
        ...
        # hidden_states = torch.zeros(self.num_recurrent_layers, x.shape[0], self.hidden_size).to(self.device)
        ...
        for i in range(len(has_zeros) - 1):
            ...
            x_ = self.encoder(x[start_idx:end_idx][0])[None]
    ...
    def forward(self, x, masks, hidden_states=None):
        if hidden_states == None:
            hidden_states = torch.zeros(self.num_recurrent_layers, x.shape[0], self.hidden_size).cuda()
            x = x.transpose(1,0).reshape(-1, *x.shape[2:])
            masks = masks.transpose(1,0).reshape(-1)
