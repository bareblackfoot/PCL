...
class MoCo(nn.Module):
    ...
    def forward(self, im_q, im_soft=None, im_k=None, is_eval=False, cluster_result=None, index=None):
        """
        Input:
        ...
        n = self.encoder_q(im_soft)  # queries: NxC
        n = nn.functional.normalize(n, dim=1)
        l_pos_soft = torch.einsum('nc,nc->n', [n, k]).unsqueeze(-1)
        l_neg_soft = torch.einsum('nc,ck->nk', [n,  self.queue.clone().detach()])
